<script lang="ts">
  import type { IndexDiff } from '$lib/types/diff';

  let { diffs }: { diffs: IndexDiff[] } = $props();
...
<div class="indexes-diff">
  {#if diffs.length === 0}
    <div class="empty">No indexes to compare</div>
  {:else}
      ...
      <tbody>
        {#each diffs as diff}
          <tr class={statusClass(diff.status)}>
            <td class="idx-name">{diff.name}</td>
            <td>{diff.source?.columns.join(', ') ?? '—'}</td>
            <td>{diff.target?.columns.join(', ') ?? '—'}</td>
            <td>{diff.source?.is_unique ?? '—'} / {diff.target?.is_unique ?? '—'}</td>
            <td>{diff.source?.index_type ?? '—'} / {diff.target?.index_type ?? '—'}</td>
