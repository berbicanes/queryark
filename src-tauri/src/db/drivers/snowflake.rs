// Snowflake driver — REST-based, requires snowflake-api crate.
// Enabled via the "snowflake" feature flag.
...
impl SnowflakeDriver {
    pub async fn connect(config: &ConnectionConfig) -> Result<Self, AppError> {
        let _account = config.snowflake_account.as_deref().ok_or_else(|| {
            AppError::InvalidConfig("Snowflake account is required".to_string())
        })?;
        Err(AppError::Database(
            "Snowflake driver requires the 'snowflake' feature flag to be enabled.".to_string(),
        ))
    }
}
...
impl DbDriver for SnowflakeDriver {
    ...
    async fn execute_raw(&self, _sql: &str) -> Result<QueryResponse, AppError> { Err(AppError::UnsupportedOperation("Snowflake not configured".to_string())) }
    ...
    async fn get_item_data(&self, _container: &str, _item: &str, _limit: i64, _offset: i64) -> Result<QueryResponse, AppError> { Err(AppError::UnsupportedOperation("Snowflake not configured".to_string())) }
...
impl SqlDriver for SnowflakeDriver {
    ...
    async fn get_table_data(&self, _schema: &str, _table: &str, _limit: i64, _offset: i64) -> Result<QueryResponse, AppError> { Err(AppError::UnsupportedOperation("Snowflake not configured".to_string())) }
    async fn get_row_count(&self, _schema: &str, _table: &str) -> Result<i64, AppError> { Ok(0) }
    async fn update_cell(&self, _schema: &str, _table: &str, _column: &str, _value: &str, _pk_columns: Vec<String>, _pk_values: Vec<String>) -> Result<(), AppError> { Err(AppError::UnsupportedOperation("Snowflake not configured".to_string())) }
    async fn insert_row(&self, _schema: &str, _table: &str, _columns: Vec<String>, _values: Vec<String>) -> Result<(), AppError> { Err(AppError::UnsupportedOperation("Snowflake not configured".to_string())) }
    async fn delete_rows(&self, _schema: &str, _table: &str, _pk_columns: Vec<String>, _pk_values_list: Vec<Vec<String>>) -> Result<u64, AppError> { Err(AppError::UnsupportedOperation("Snowflake not configured".to_string())) }
}
